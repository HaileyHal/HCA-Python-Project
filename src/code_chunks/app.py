...
st.title('Hurricane HCA Employee Response')
import pandas as pd
hca = pd.read_csv("CoordHCA.csv")

destinationZip = st.number_input("What is your facility zipcode? (Chelsea/TAs please use 77004. This is a Houston Hospital Facility): ",  format="%d") # determining what hospital we are going to
destinationZip = int(destinationZip)
...
hurricaneLvl = st.number_input("What is the incoming hurricane level? (1-5): ",  format="%d")
if not hurricaneLvl:
    st.stop()

if hurricaneLvl == 1: # Make depending on hurricane level
    neededEmps = empCount.get(destinationZip) * 0.1
elif hurricaneLvl == 2:
    neededEmps = empCount.get(destinationZip) * 0.2
elif hurricaneLvl == 3:
    neededEmps = empCount.get(destinationZip) * 0.3
elif hurricaneLvl == 4:
    neededEmps = empCount.get(destinationZip) * 0.4
elif hurricaneLvl == 5:
    neededEmps = empCount.get(destinationZip) * 0.5
neededEmps = round(neededEmps, 0) # change to round
st.write(f"You will need {neededEmps} employees brought in.")

destination = st.text_input("What is your destination airport? Please enter the airport code: (Chelsea/Tas please use HOU. This is the Houston Airport Code): ")
    ...
    try: # what I want to happen
        ...
        for flight in flights: # for each origin/departure airport:
            ...
            if all_prices[origin]: # Only calculate average if we collected prices
                ...
                st.write(f"The average price for {origin} to {destination} on {today} is ${avg_price}")
            else:
...
for index, row in prices_sorted.iterrows(): # this will look at one origin airport at a time, looking at the top one first, bc they were sorted by price before
    while neededEmps > 0: # while more employees are needed, this will run 
        ...
        st.write(f"There are {totalAvEmps} employees available from {row['origin']}.")
        ...
        if neededEmps > totalAvEmps: # if the destination hospital needs more emps than what was available
            ...
            st.write(f"You need {totalAvEmps} employees from the {row['origin']} airport. This will cost ${totalPrice}.")
        ...
        response = st.text_input("Would you like to see which hospitals these employees are coming from? (Y/N): ") # for project purposes: allowing hospital managers to see what hospitals specifically they can pull from
        if not response:
            st.stop()
        if response == "Y":
            st.write(f"These are the hospitals within 50 miles of the {row['origin']} airport, and how many employees work at each one:")
            st.write(radiusHos.groupby("facility_zip")["Emp34Id"].nunique()) # showing hospital zips and how many emps they have
...
st.write(f"Your total cost is ${finalPrice}.")
